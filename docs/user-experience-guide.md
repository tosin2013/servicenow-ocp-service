...
```mermaid
flowchart TD
    StartDeploy[Click "Deploy Application"] --> ChooseSource{Choose Deployment Source}
    
    ChooseSource -->|Container Image| ImageDeploy[Container Image Deployment]
    ChooseSource -->|Git Repository| GitDeploy[Git Repository Deployment]
    ChooseSource -->|YAML/Template| YAMLDeploy[YAML Template Deployment]
    ChooseSource -->|Sample App| SampleDeploy[Sample Application]
    
    ImageDeploy --> ImageForm[Image Configuration Form]
    GitDeploy --> GitForm[Git Repository Form]
    YAMLDeploy --> YAMLEditor[YAML Editor]
    SampleDeploy --> SampleSelector[Sample Application Selector]
    
    ImageForm --> ResourceConfig[Resource Configuration]
    GitForm --> BuildConfig[Build Configuration]
    YAMLEditor --> ValidateYAML[Validate YAML]
    SampleSelector --> ResourceConfig
    
    BuildConfig --> ResourceConfig
    ValidateYAML --> ResourceConfig
    
    ResourceConfig --> NetworkConfig[Network Configuration]
    NetworkConfig --> SecurityConfig[Security Configuration]
    SecurityConfig --> ReviewDeploy[Review Deployment]
    
    ReviewDeploy --> DeploymentOK{Configuration Valid?}
    DeploymentOK -->|No| ResourceConfig
    DeploymentOK -->|Yes| StartDeployment[Start Deployment]
    
    StartDeployment --> MonitorDeploy[Monitor Deployment Progress]
    MonitorDeploy --> DeployComplete[Deployment Complete]
    DeployComplete --> AppRunning[Application Running]
    
    style StartDeploy fill:#e3f2fd
    style AppRunning fill:#c8e6c9
    style DeploymentOK fill:#fff3e0
```
